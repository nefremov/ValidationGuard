...
ALWAYS run `python3 config/kits/sdlc/scripts/pr.py list` WHEN user intent matches PR list patterns:
...
1. Run `python3 config/kits/sdlc/scripts/pr.py list`
...
2. Read `config/kits/sdlc/workflows/pr-review.md` and follow its steps
3. Use `python3 config/kits/sdlc/scripts/pr.py` as the script
...
7. Use templates from `config/kits/sdlc/artifacts/PR-CODE-REVIEW-TEMPLATE/template.md` and `config/kits/sdlc/artifacts/PR-STATUS-REPORT-TEMPLATE/template.md`

### PR Status Workflow

When routed to PR status:
1. **ALWAYS fetch fresh data first** — `pr.py status` auto-fetches, but never assume prior data is current
2. Read `config/kits/sdlc/workflows/pr-status.md` and follow its steps
3. Use `python3 config/kits/sdlc/scripts/pr.py` as the script
4. When target is `ALL` or no PR number given, run `pr.py list` first to show available PRs
...
1. Read `config/kits/sdlc/workflows/migrate-openspec.md` and follow its steps
